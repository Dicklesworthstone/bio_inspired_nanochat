...
- 2026-09-02: `GPTSynaptic.chunked_train_step` and `--hebb_chunk_len` (`base_train`, `eval_matrix --hebb-chunk-len`) add the training half of the regime: the batch is read chunk by chunk through a training-only cache, each chunk's token-weighted loss is back-propagated before the next chunk's deferred writes land, and the cache is detached between chunks (truncated BPTT). The inference `KVCache` cannot serve here because it mutates one shared buffer in place. `tests/test_chunked_training_regime.py` (bead `hwxb.8`).

### Matrix launcher
- 2026-09-02: `scripts/matrix_launch.py --columns a,b` selects explicit cells (re-runs, smoke), and `tests/test_e2e_matrix_pipeline.py` proves the whole D1 chain at toy scale as subprocesses: launcher → `base_train` checkpoints under the tag `eval_matrix` resolves → `eval_matrix matrix --checkpoint-dir` scoring from checkpoint metadata → `eval_stats` pairing (nightly; e2e+slow).
