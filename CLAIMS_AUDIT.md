...
> **The one finding that dominates everything below (2026-09-01):** every mechanism is real and unit-tested, and no model larger than 2 layers × 64 dims has ever been trained. `results/registry.jsonl` is 100% `cpu:x86_64` and has no `val_bpb` row. The project has no evidence yet that biology helps a language model; the two directional toy-scale findings are unmeasured-not-null (online fast-weights ON = OFF to six decimals, but measured in a single-forward regime that cannot see the writes; the chunked regime exists since 2026-09-02 and the deciding run is `hwxb.9`) and negative (split/merge NAS regressed loss on 8/8 seeds; re-derived 2026-09-02 under deterministic evaluation, identical to all digits). The synaptic path is 4–18× slower than vanilla on CPU. The pre-registered experiment that produces the first signal is `docs/ablation_matrix.md` (beads `hwxb.3`–`hwxb.6`), gated on the dual-RTX-4090 host.
