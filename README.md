...
**Measured (2026-09-01, toy scale):** the health signal does not yet support that story. Utilization is the routed-token fraction, so a uniformly used expert sits at `top_k / num_experts`, while the thresholds are absolute: at 4 experts nothing ever fires (health ≈ 0.3 sits between the merge and split thresholds), and at the 8-expert default every expert is a merge candidate on every check. Because energy relaxes toward `1 − utilization`, health ≈ `u(1 − u)` at steady state and peaks at half the routing mass, so a monopolising expert reads as *dead* and the split threshold of 0.80 is unreachable for any expert count. A scale-free alternative ships default-off: `--sm_health_mode=relative` scores each expert by utilization relative to the fair share `top_k / num_experts` (1.0 = uniform, >1 overworked, <1 underused), with the three thresholds then given in those units (e.g. `--split_health_min=1.5 --merge_health_max=0.35 --reset_health_max=0.05`). Whether lifecycle events *help* under either signal is still the open question on bead `sx1m`; until it is answered, treat the lifecycle as an opt-in study, not a working capacity allocator. **Pilot (2026-09-02, `uta.8`):** a 2-seed CPU run of the structural pair (8 experts, top-2, 120 steps, controller every 10 steps, `results/structural_pair_pilot_2026-09-02.json`) fired zero splits, merges or resets under *either* signal, with loss identical to the no-controller arm: utilization stayed within ±0.03 of the 0.25 fair share because the balance loss (`moe_balance_loss=0.01`) drives routing toward uniform, which is exactly the quantity both health modes key on. A utilization-triggered lifecycle and a load-balance loss work against each other. A follow-up with the balance loss switched off (`--balance-loss 0.0`, same recipe, `results/structural_pair_pilot_2026-09-02_balance0.json`) still fired zero events in all six arms, with utilization spread across the eight experts of 0.03–0.04 around the 0.25 fair share (±15%), so a weaker balance loss alone does not wake the lifecycle at this scale either: the utilization EMA (rate decoded from the genome, bounds 0.001–0.03) moves slowly and 120 steps of a fresh router leave routing near uniform. The D1 structural arm needs a demand signal that is not a slow utilization average before it can measure anything. Since 2026-09-03 that signal exists as `--sm_health_mode=credit` (bead `uta.9`): NeuroScore's per-expert gradient credit relative to the layer mean drives splits and resets (thresholds in the same units as `relative`; the controller fails loudly if NeuroScore has not been stepped). Whether it fires in ordinary training is being measured (`results/structural_pair_pilot_2026-09-02_credit.json`); until that artifact exists, treat it as built, not shown to act.
